...
string getCourseCode(const string& request) {
	while (true) {
		...
		if (isNumber(courseCode.substr(0, 5)) && courseCode.at(5) == ' ')	// does courseCode start with a 5 digit number and then a space?
			return courseCode;
		else
...
int main() {
	...
	do {
		stream << degreeCode << string(" ") << getCourseCode("Enter a course code and title: ");	// build a courseTitle 
		getline(stream, courseTitle);			// pass the stream into a string
		courseTitles.push_back(courseTitle);	// push the string into a vector
		stream.clear();							// clear the stream for the next use

		//are you finished?
		bool success{ false };
		do {
			finished = getInput("Finished? (y/n): ");
			if (finished == "Y" || finished == "y" || finished == "N" || finished == "n")	// inculded captial y/n 
				success = true;
			else
