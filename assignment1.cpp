...
float photonEnergy(int atomicNumber, int energyLevel_1, int energyLevel_2, bool inJoules) { // inJoules is true if output in J, false if in eV
	...
	float ret{ (energyRydberg * (atomicNumber * atomicNumber) * (1 / float(energyLevel_2 * energyLevel_2) - 1 / float(energyLevel_1 * energyLevel_1))) };

	return ret;
}
//TODO compare e levels
...
bool stringToInt(string& _string, int& _integer) {	
...
int main() {
	...
	do {
		...
		// declare variables - could make the unsigned 
		...
		do {
			if (stringToInt(getInput(string("Enter fianl energy level: ")), energyLevel_2)) {
				if (energyLevel_2 > 0 && energyLevel_2 <= energyLevel_1) // energy level can't be 0
					success = true;
				else
		...
		float energy{ photonEnergy(atomicNumber, energyLevel_1, energyLevel_2, answer_Je == "J") };

		cout << energy << "\n";
		printf("Energy of the photon : %3.2e %s \n", energy, (answer_Je == "J" ? "J" : "eV"));
